...
void loop() {
  ...
  USBSerial.print("All Frequencies in Queue: ");
  cppQueue tempQueue(sizeof(float), 10, FIFO); // Temporary queue

  // Transfer elements from freqQueue to tempQueue and print them
  //Holds FIFO
  while (!freqQueue.isEmpty()) {
    float freq;
    freqQueue.peek(&freq); // Peek at the element without removing it
    USBSerial.print(freq, 2);
    USBSerial.print(", ");
   tempQueue.push(&freq); // Push the element to tempQueue
    freqQueue.pop(&freq);  // Remove the element from freqQueue
  }

  // Transfer elements back to freqQueue from tempQueue
  while (!tempQueue.isEmpty()) {
    float freq;
    tempQueue.pop(&freq);  // Remove the element from tempQueue
    freqQueue.push(&freq); // Push the element back to freqQueue
  }
  USBSerial.println();


 if (!freqQueue.isEmpty()) { //reads the first element in the queue, maps to color
    float freq1;
