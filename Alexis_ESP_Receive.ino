...
float receivedArray[10];
float lastReceivedArray[10];
...
void loop() {
 ...
 if (Serial1.available() >= 10*sizeof(float)) {
    Serial1.readBytes((byte*)receivedArray, 10*sizeof(float));
    USBSerial.print("Average Dominant Frequencies (Hz): ");

    for (int i = 0; i < 10; i++) { 
      USBSerial.print(receivedArray[i]); // Print with 2 decimal places
      if (i < 9) {
        USBSerial.print(", "); // Print a comma and space for all but the last element
      }

  }
    USBSerial.println(); //new line 

    //practice color
    int hue = map(receivedArray[0], 0, SAMPLES / 2, 0, 255); // Map frequency to hue (0-255)
    color = matrix.colorHSV(hue * 256, 255, 255); // Convert hue to RGB color
  }
